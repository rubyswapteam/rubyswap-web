...
exports.MarketplaceProvider = function (_a) {
  ...
  var _c = react_1.useState(undefined),
      collectionTrades = _c[0],
      setCollectionTrades = _c[1];

  var _d = react_1.useState(undefined),
      userTradesFiltered = _d[0],
      setUserTradesFiltered = _d[1];

  var _e = react_1.useState(undefined),
      recentTrades = _e[0],
      setRecentTrades = _e[1];
  ...
  var _f = react_1.useState({
    ...
    contract: ''
      ...
      tradeFilters = _f[0],
      setTradeFilters = _f[1];

  var _g = react_1.useState(undefined),
      activeCollection = _g[0],
      setActiveCollection = _g[1];
  ...
  function getCollectionTrades(collection) {
    ...
    if (collection === void 0) {
      collection = activeCollection === null || activeCollection === void 0 ? void 0 : activeCollection.contractAddress;
    }
      ...
      return __generator(this, function (_d) {
        switch (_d.label) {
          case 0:
            ...
            return [4
            /*yield*/
            , Promise.all([getCollectionTradesX2Y2(collection), getCollectionTradesLooksRare(collection)])];

          case 1:
            _c = _d.sent(), x2y2res = _c[0], looksRes = _c[1];
            trades = [];
            if (x2y2res) trades = __spreadArrays(trades, x2y2res);
            if (looksRes) trades = __spreadArrays(trades, looksRes);
            setCollectionTrades(trades);
            recentTrades = trades.sort(function (a, b) {
          ...
          case 2:
            if (!(i < recentTrades.length)) return [3
            /*break*/
            , 6];
            trade = recentTrades[i];
            if (!!trade.image) return [3
            /*break*/
            , 5];
  ...
  function applyTradeFilters(trades, newFilter) {
    ...
    newFilter = typeof newFilter !== 'undefined' ? newFilter : __assign({}, tradeFilters);
  ...
  function applyTradeFilter(obj, field, filters) {
    return filters[field] !== '' ? obj.filter(function (order) {
      return order[field] === filters[field];
    }) : obj;
  }
  ...
  function getAggregateTrades(x2y2res, looksRes) {
    var userSales = [];
    var userPurchases = [];
    if (x2y2res.sales && x2y2res.sales.length > 0) userSales = __spreadArrays(userSales, x2y2res.sales);
    if (x2y2res.purchases && x2y2res.purchases.length > 0) userPurchases = __spreadArrays(userPurchases, x2y2res.purchases);
    if (looksRes.sales && looksRes.sales.length > 0) userSales = __spreadArrays(userSales, looksRes.sales);
    if (looksRes.purchases && looksRes.purchases.length > 0) userPurchases = __spreadArrays(userPurchases, looksRes.purchases);
    return {
      sales: userSales,
  ...
  function getCollectionBySlug(slug, getTrades, setActive, persist) {
    ...
    return __awaiter(this, void 0, void 0, function () {
      var collectionRaw, nftCollection, _a;
      ...
      return __generator(this, function (_b) {
        switch (_b.label) {
          case 0:
            ...
            return [4
            /*yield*/
            , fetch("/.netlify/functions/getDbCollectionBySlug?slug=" + slug, {
              method: 'GET',
              redirect: 'follow'
            })];
          ...
          case 2:
            ...
            if (!(collectionRaw.status != 200)) return [3
            /*break*/
            , 5];
            return [4
            /*yield*/
          ...
          case 5:
            if (slug.length > 1 && collectionRaw && collectionRaw.collection) {
              ...
              if (setActive) setActiveCollection(nftCollection);

              if (getTrades) {
                getCollectionTrades(collectionRaw.collection.primary_asset_contracts[0].address);
              }

              if (persist) {// Push to DB but stringify traits
              }
  ...
  }, [userTrades, setUserTrades, getUserTrades, tradeFilters, setTradeFilters, applyTradeFilters, userTradesFiltered, setUserTradesFiltered, activeCollection, getCollectionBySlug, collectionTrades, getCollectionTrades, recentTrades]);
  return react_1["default"].createElement(MarketplaceProviderContext.Provider, {
    value: contextValue
  }, children);
};
