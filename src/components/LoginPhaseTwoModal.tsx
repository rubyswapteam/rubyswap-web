...
export default function LoginPhaseTwoModal(props: any) {
  ...
  const verify = async () => {
    ...
    if (
      ...
      props.setIsVerified('verified');
    } else {
      setErrorMsg("Sorry, you don't have access to Ruby at the moment.");
    }
  };
  useEffect(() => {
    verify();
  ...
  return (
    <>
      {!errorMsg && (
        <div className="text-white/70 text-md py-3 flex">
          <svg
            aria-hidden="true"
          ...
          Checking for partner NFTs
        </div>
      )}
      {errorMsg && (
        <>
          <p className="text-left my-3 text-white">{errorMsg}</p>
          <p className="text-left my-3 text-white">
            Please refresh if you wish to try again.
          </p>
        </>
      )}
    </>
