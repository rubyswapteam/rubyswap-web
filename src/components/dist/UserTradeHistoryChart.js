...
function UserTradeHistoryChart(props) {
    ...
    function manipulateData(dates, trim) {
        var data = props.userTrades;
        if (data && data.sales && data.purchases) {
            var minimumDate_1 = Math.min.apply(Math, dates.daysUnix);
            var sales = data.sales
                .filter(function (trade) { return trade.timestamp > minimumDate_1; })
                .map(function (trade) { return [
                (trade.timestamp - minimumDate_1) / 86400 / trim,
                Number(trade.price),
            ]; });
            ...
            series: [
                {
                    name: 'Sales',
                    color: '#33333399',
                    data: trades === null || trades === void 0 ? void 0 : trades.sales
                },
                {
                    name: 'Purchases',
                    color: 'rgb(59, 130, 246)',
                    data: trades === null || trades === void 0 ? void 0 : trades.purchases
                },
            ],
            legend: {
                itemStyle: {
