...
export async function handler(event) {
  const st = event.queryStringParameters.searchTerm;
  const API_ENDPOINT = `https://mqxsyzoydluqyuigceuy.supabase.co/rest/v1/CollectionDetails?select=slug,name,contractAddress,imageUrl,totalSupply,osVerificationState,osFloorPrice&or=(slug.ilike.*${st}*,contractAddress.ilike.*${st}*,name.ilike.*${st}*)&order=osTotalVolume.desc`;
