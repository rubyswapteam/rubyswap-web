...
export async function handler(event) {
  if (!event.queryStringParameters.address)
    return { statusCode: 500, error: 'No wallet provided' };

  const API_ENDPOINT = `https://mqxsyzoydluqyuigceuy.supabase.co/rest/v1/Watchlists?select=watched&address=ilike.${event.queryStringParameters.address}`;
